...
def training(epochs=600, numUpdates=5, epsilon = 0.2, batchSize=1024, subBatchSize=64):
    ...
    optimizerActor = optim.Adam(actor.parameters(), lr=3e-4)
    optimizerCritic = optim.Adam(critic.parameters(), lr=3e-4)


    for epoch in tqdm(range(epochs)):
         batchStates, batchActions, batchLogProbs, batchRewardsToGo = getRollout(actor, env, logStd, timeStepsPerBatch=batchSize)
